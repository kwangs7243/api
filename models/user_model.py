...
class UserModel:
    def check_id_duplication(self, dto):  #  아이디 중복 체크
        with Dbmanager() as cursor:
            sql = "SELECT login_id From users WHERE login_id = %s"
            cursor.execute(sql, (dto.login_id,))
            result = cursor.fetchone()
        if result:
            return True
        else:
            return False
    def sign_up(self, dto): # 회원가입
        with Dbmanager(commit=True) as cursor:
            sql = "INSERT INTO users (login_id, passwd, name) VALUES (%s, %s, %s)"
            cursor.execute(sql, (dto.login_id, dto.passwd, dto.name))
    def sign_in(self, dto): # 로그인
        with Dbmanager() as cursor:
            sql = "SELECT id,login_id FROM users WHERE login_id = %s AND passwd = %s"
            cursor.execute(sql, (dto.login_id, dto.passwd))
            result = cursor.fetchone()
